...
class AltmetricsForDoi(object):
    def __init__(self, doi):
        self.doi = doi
        self.ced_url = "https://api.eventdata.crossref.org/v1/events?rows=1000&filter=from-collected-date:1990-01-01,until-collected-date:2099-01-01,obj-id:{}".format(
            self.doi
        )
        self.events = []
        self.sources = []
...
class OaDoi(object):
    ...
    def get(self):
        r = requests_retry_session(retries=2).get(
            self.url + "?email=team+paperbuzz@ourresearch.org", timeout=10
        )
